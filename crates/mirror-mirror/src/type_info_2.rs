...
pub mod graph {
    ...
    impl StructInfoNode {
        pub fn new<T>(fields: &[NamedFieldNode]) -> Self
        where
            T: Typed,
        {
            Self {
    ...
    impl TupleStructInfoNode {
        pub fn new<T>(fields: &[UnnamedFieldNode]) -> Self
        where
            T: Typed,
        {
            Self {
    ...
    impl EnumInfoNode {
        pub fn new<T>(variants: &[VariantNode]) -> Self
        where
            T: Typed,
        {
            Self {
    ...
    impl StructVariantInfoNode {
        pub fn new(name: &'static str, fields: &[NamedFieldNode]) -> Self {
            Self {
                name: name.to_owned(),
                fields: fields.to_vec(),
            }
    ...
    impl TupleVariantInfoNode {
        pub fn new(name: &'static str, fields: &[UnnamedFieldNode]) -> Self {
            Self {
                name: name.to_owned(),
                fields: fields.to_vec(),
            }
    ...
    impl NamedFieldNode {
        pub fn new<T>(name: &'static str, graph: &mut TypeInfoGraph) -> Self
        where
            T: Typed,
        {
            Self {
    ...
    impl UnnamedFieldNode {
        pub fn new<T>(graph: &mut TypeInfoGraph) -> Self
        where
            T: Typed,
        {
            Self {
            ...
            graph.get_or_build_with::<Self, _>(|graph| {
                EnumInfoNode::new::<Self>(&[
                    VariantNode::Tuple(TupleVariantInfoNode::new(
                        "Some",
                        &[UnnamedFieldNode::new::<T>(graph)],
                    )),
                    VariantNode::Unit(UnitVariantInfoNode::new("None")),
                ])
            })
        }
    }
...
mod tests {
    ...
    fn struct_() {
        ...
        impl Typed for Foo {
            fn build(graph: &mut TypeInfoGraph) -> Id {
                graph.get_or_build_with::<Foo, _>(|graph| {
                    StructInfoNode::new::<Foo>(&[
                        NamedFieldNode::new::<i32>("n", graph),
                        NamedFieldNode::new::<Vec<Foo>>("foos", graph),
                    ])
                })
            }
        }

        let type_info = Foo::type_info();

        assert_eq!(
            type_info.type_().type_name().unwrap(),
            "mirror_mirror::type_info_2::tests::struct_::Foo"
        );
            ...
            match field.name() {
                "foos" => {
                    assert_eq!(
                        ...
                        "alloc::vec::Vec<mirror_mirror::type_info_2::tests::struct_::Foo>"
                    );
    ...
    fn enum_() {
        ...
        impl Typed for Foo {
            fn build(graph: &mut TypeInfoGraph) -> Id {
                graph.get_or_build_with::<Foo, _>(|graph| {
                    EnumInfoNode::new::<Foo>(&[
                        VariantNode::Struct(StructVariantInfoNode::new(
                            "A",
                            &[NamedFieldNode::new::<String>("a", graph)],
                        )),
                        VariantNode::Tuple(TupleVariantInfoNode::new(
                            "B",
                            &[UnnamedFieldNode::new::<Vec<Foo>>(graph)],
                        )),
                        VariantNode::Unit(UnitVariantInfoNode::new("C")),
                    ])
                })
            }
        }
